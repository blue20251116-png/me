...
function douyinNormalize(raw){
  ...
  return {
    ...
    authorId:String(first(author.secUid,author.uid,author.id,x.authorId,'')),
    authorName:String(first(author.nickname,author.name,x.authorName,'')),
    publishedAt:first(x.createTime,x.create_time,x.publishedAt,null),
    views:num(first(stat.playCount,stat.play_count,x.views,x.playCount,0)),
    likes:num(first(stat.diggCount,stat.likeCount,stat.digg_count,x.likes,x.likeCount,0)),
    comments:num(first(stat.commentCount,stat.comment_count,x.comments,0)),
    shares:num(first(stat.shareCount,stat.share_count,x.shares,0)),
    metadata:raw
  };
}
function xhsSearchNormalize(raw){
  ...
  const user=card.user||{};const interact=card.interact_info||card.interactInfo||{};const token=first(card.xsec_token,card.xsecToken,user.xsec_token,user.xsecToken,'');
  const sourceUrl=first(x.postUrl,card.postUrl,raw?.postUrl,`https://www.xiaohongshu.com/explore/${id}${token?`?xsec_token=${encodeURIComponent(token)}`:''}`);
  return {
    externalPostId:id,sourceUrl,
    videoUrl:videoUrlFrom(card),thumbnailUrl:imageUrlFrom(card),
    caption:String(first(card.display_title,card.displayTitle,card.title,x.title,'')),
    authorId:String(first(user.user_id,user.userId,user.id,'')),
    authorName:String(first(user.nick_name,user.nickName,user.nickname,user.name,'')),
    publishedAt:first(card.time,card.timestamp,x.scrapedAt,null),
    views:num(first(interact.view_count,interact.viewCount,card.views,0)),
    likes:num(first(interact.liked_count,interact.likedCount,card.likes,0)),
    comments:num(first(interact.comment_count,interact.commentCount,card.comments,0)),
    shares:num(first(interact.share_count,interact.shareCount,card.shares,0)),
    metadata:raw
  };
}
...
export class ApifyDouyinCollector extends BaseSocialCollector{
  ...
  async discover(target){
    ...
    const items=await runActor(DOUYIN_ACTOR,{keywords:[keyword],maxResultsPerQuery:10,sort:'most_liked',publishTime:'one_week',duration:'under_1m',shouldDownloadVideos:false});
