...
export async function runSocialMonitorOnce(){
  const targets=db.prepare('SELECT * FROM social_monitor_targets WHERE enabled=1 ORDER BY last_checked_at IS NOT NULL,last_checked_at ASC').all();
  let discovered=0;
  for(const target of targets){
    try{
      const collector=collectorFor(target.platform);
      ...
      console.error(`[SocialMonitor] platform=${target.platform} target=${target.target_value} reason=${String(err?.message||err)}`);
    }
  }
  return {targets:targets.length,discovered};
}
export function startSocialMonitor(){
  ...
  const tick=()=>runSocialMonitorOnce().catch(err=>console.error('[SocialMonitor]',err));
