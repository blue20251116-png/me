...
function encrypt(value){
  ...
  const tag=cipher.getAuthTag();return Buffer.concat([iv,tag,enc]).toString('base64');
}
function decrypt(payload){
  const b=Buffer.from(payload,'base64'),iv=b.subarray(0,12),tag=b.subarray(12,28),enc=b.subarray(28);
  const decipher=crypto.createDecipheriv('aes-256-gcm',masterKey(),iv);decipher.setAuthTag(tag);
...
export function setAdminPin(pin){
  if(!/^.{6,}$/.test(String(pin||''))) throw new Error('관리자 PIN은 6자 이상이어야 합니다.');
  if(hasAdminPin()) throw new Error('관리자 PIN이 이미 설정되어 있습니다.');
  const salt=crypto.randomBytes(16).toString('hex');
  db.prepare('INSERT OR REPLACE INTO app_settings(key,value,updated_at) VALUES(?,?,CURRENT_TIMESTAMP)').run('ADMIN_PIN_SALT',salt);
  db.prepare('INSERT OR REPLACE INTO app_settings(key,value,updated_at) VALUES(?,?,CURRENT_TIMESTAMP)').run('ADMIN_PIN_HASH',pinHash(pin,salt));
}
export function verifyAdminPin(pin){
  const s=db.prepare("SELECT value FROM app_settings WHERE key='ADMIN_PIN_SALT'").get()?.value;
  const h=db.prepare("SELECT value FROM app_settings WHERE key='ADMIN_PIN_HASH'").get()?.value;
  if(!s||!h)return false;const got=pinHash(pin,s);
