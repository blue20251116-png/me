...
async function analyzePost(postId){
  console.log(`[DiscoveryAuto][ANALYZE] start post=${postId}`);
  ...
  if(!r.ok){
    console.error(`[DiscoveryAuto][ANALYZE][FAILED] post=${postId} status=${r.status} reason=${text.slice(0,1200)}`);
    throw new Error(text||`분석 실패 (${r.status})`);
  }
  const parsed=JSON.parse(text||'{}');
  console.log(`[DiscoveryAuto][ANALYZE] success post=${postId} status=${parsed.status||'OK'} matches=${Array.isArray(parsed.matches)?parsed.matches.length:0}`);
  return parsed;
}
...
async function executeOneClick(){
  ...
  try{
    ...
    console.log(`[DiscoveryAuto] done processed=${runState.processed} success=${runState.succeeded} failed=${runState.failed}`);
  }catch(err){runState.stage='FAILED';runState.lastError=String(err?.message||err);runState.message=`자동 실행 실패 · ${runState.lastError}`;console.error(`[DiscoveryAuto][FAILED] ${runState.lastError}`,err);}
...
discoveryAutoRouter.get('/status',(_req,res)=>{
  ...
  res.json({collectors,ready:collectors.DOUYIN||collectors.XIAOHONGSHU,services:{apify:collectors.APIFY,openai:!!getSecret('OPENAI_API_KEY'),serpapi:!!getSecret('SERPAPI_API_KEY'),coupang:!!getSecret('COUPANG_ACCESS_KEY')&&!!getSecret('COUPANG_SECRET_KEY'),publicBaseUrl:!!getSecret('PUBLIC_BASE_URL')},activeTargets:Number(db.prepare('SELECT COUNT(*) FROM social_monitor_targets WHERE enabled=1').pluck().get()||0),posts:Number(db.prepare('SELECT COUNT(*) FROM social_posts').pluck().get()||0),run:{...runState}});
});
...
discoveryAutoRouter.post('/run',(req,res)=>{
  ...
  if(!getSecret('OPENAI_API_KEY'))return res.status(503).json({error:'OpenAI API Key를 먼저 저장해 주세요. 상품 판독에 필요합니다.'});
