...
async function refreshAutoState({refreshResults=false}={}){
  ...
  const missing=[];if(!s.instagramTargets)missing.push('벤치마킹 계정');if(!(s.services?.gemini??s.services?.openai))missing.push('Gemini');if(!s.services?.serpapi)missing.push('SerpApi');if(!s.services?.coupang)missing.push('쿠팡 API');if(!s.services?.publicBaseUrl)missing.push('Public URL');
...
window.addEventListener('message',event=>{
  ...
  if(msg.type==='STARTED'){
    ...
    setAutoStatus('Chrome Bridge 실행 시작됨 · 기존 결과는 숨기고 이번 실행 결과만 기다리는 중','ok');
    return;
  }
  if(msg.type==='RESULT'){
    ...
    setAutoStatus(`수집 완료 · 계정 ${r.targets||0}개 확인 · 후보 ${r.candidates||0}개 · ME 저장 ${r.selected||0}개 · 이번 실행 결과만 표시 중`,'ok');
...
if(autoBtn){
  autoBtn.addEventListener('click',async()=>{
    try{
      ...
      if(!s.instagramTargets||!(s.services?.gemini??s.services?.openai)||!s.services?.serpapi||!s.services?.coupang||!s.services?.publicBaseUrl)return;
      if(typeof window.prepareDiscoveryBatch==='function')await window.prepareDiscoveryBatch();
      autoBtn.disabled=true;autoBtn.textContent='Chrome에 작업 요청 중…';setAutoStatus('기존 결과를 숨겼습니다 · 새 Instagram 수집을 시작합니다','ok');
