...
function videoUrl(kind,id){return kind==='clean'?`/storage/clean/${encodeURIComponent(id)}-clean-v2.mp4`:`/storage/social/${encodeURIComponent(id)}.mp4`}
function mediaBox(title,url,note,fileName){return `<div class="video-box"><h4>${esc(title)}</h4><video controls preload="metadata" src="${esc(url)}"></video><div class="video-actions"><a class="btn-primary" href="${esc(url)}" download="${esc(fileName)}">⬇ MP4 다운로드</a><a class="btn-secondary" href="${esc(url)}" target="_blank" rel="noopener">새 탭에서 열기</a></div><div class="muted">${esc(note)}</div></div>`}
